...
public class CellTest {
    ...
    @Test
    @DisplayName("dataFormatのテスト")
    public void cell_data_format() throws Exception {
        // 定義ファイルの読み込み
        ClassLoader classLoader = getClass().getClassLoader();
        InputStream in = classLoader.getResource("writer/cell/cell_data_format.xml").openStream();
        ...
        // 書き込む内容の設定
        JsonNode jsonNode = new JsonNode();
        jsonNode.putVar("boolean_value1", new JsonObject(true));
        jsonNode.putVar("boolean_value2", new JsonObject("true"));
        jsonNode.putVar("date_value1", new JsonObject(LocalDate.of(2025, 10, 27)));
        jsonNode.putVar("date_value2", new JsonObject("2025-10-27"));
        jsonNode.putVar("datetime_value1", new JsonObject(LocalDateTime.of(2025, 10, 27, 9, 41, 30)));
        jsonNode.putVar("datetime_value2", new JsonObject("2025-10-27T09:41:30.000"));
        jsonNode.putVar("formula_value", new JsonObject("H2 + J2"));
        jsonNode.putVar("numeric_value1", new JsonObject(12345.12));
        jsonNode.putVar("numeric_value2", new JsonObject("12345.12000"));
        jsonNode.putVar("string_value", new JsonObject("テスト"));
        ...
        FormaDiffer differ = new FormaDiffer();
        FileInputStream comparing = new FileInputStream(absolutePath);
        InputStream compared = classLoader.getResource("writer/cell/cell_data_format.xlsx").openStream();
        JsonObject jsonObject = differ.diff(comparing, compared);
        logger.log(Level.INFO, "比較結果: " + new JsonSerializer().serializeFromJsonObject(jsonObject));

        Assertions.assertTrue(jsonObject.isJsonNodes());
        Assertions.assertEquals(0, ((JsonNodes) jsonObject.getValue()).size());
    }
}
